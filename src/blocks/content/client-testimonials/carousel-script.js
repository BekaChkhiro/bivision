document.addEventListener('DOMContentLoaded', function() {
    ...
    const AUTO_LOOP_INTERVAL = 2500; // 2.5 seconds between slides
    ...
    function adjustCardSizes() {
        ...
        testimonialBlocks.forEach(block => {
            ...
            // Set carousel inner width to match other blocks (1250px)
            carouselInner.style.maxWidth = '1250px';
            ...
            // Get available width after setting max-width
            const availableWidth = carouselInner.clientWidth;
            
            // Calculate card width accounting for margins (3 cards side by side)
            const marginSize = 20;
            const cardWidth = Math.floor((availableWidth - (marginSize * 2)) / 3);
            ...
            cards.forEach((card, index) => {
                ...
                card.style.marginLeft = index === 0 ? '0' : `${marginSize}px`;
            });
    ...
    function setupDesktopDots() {
        ...
        testimonialBlocks.forEach(block => {
            ...
            // Calculate how many slides we need on desktop (3 testimonials per slide)
            const CARDS_PER_SLIDE = 3;
            const slideCount = Math.ceil(cards.length / CARDS_PER_SLIDE);
            ...
            dots.forEach((dot, index) => {
                ...
                newDot.addEventListener('click', () => {
                    ...
                    const CARDS_PER_SLIDE = slideType === 'mobile' ? 1 : 3;
                    ...
                    // Calculate scroll position based on the target card
            ...
            carousel.addEventListener('scroll', () => {
                ...
                carousel.scrollTimeout = setTimeout(() => {
                    ...
                    // For desktop, calculate which slide this card belongs to (3 cards per slide)
                    const CARDS_PER_SLIDE = 3;
    ...
    function setupDesktopDragScroll() {
        ...
        testimonialBlocks.forEach(block => {
            ...
            // Mouse move event - update scroll position
            window.addEventListener('mousemove', (e) => {
                if (!isDragging) return;
                
                const x = e.pageX;
                const walk = (startX - x) * 1.5; // Multiply for smoother scrolling
            ...
            const updateActiveCard = (index) => {
                if (index >= 0 && index < cards.length) {
                    ...
                    cards.forEach(card => {
                        card.style.boxShadow = '0px 4px 20px 0px rgba(0, 0, 0, 0.05)';
                        card.style.transform = '';
                    });
            ...
            // Function to snap to nearest card after momentum scrolling
            ...
            window.addEventListener('mouseup', () => {
                ...
                if (Math.abs(velocity) > 0.1) {
                    let startTime = null;
                    const duration = 500; // ms - duration of momentum animation
                    ...
                    // If velocity is too low, just snap to nearest card
                    snapToNearestCard();
                }
            });
    ...
    function initTestimonialCarousels() {
        ...
        testimonialBlocks.forEach((block, blockIndex) => {
            ...
            if (!isMobile()) {
                // Desktop setup
                carousel.style.cssText = 'display: flex !important; flex-wrap: nowrap !important; overflow-x: auto !important; scroll-snap-type: x mandatory !important; scrollbar-width: none !important; -ms-overflow-style: none !important; scroll-behavior: smooth !important; padding: 20px 0 !important; position: relative !important; left: 0 !important; max-width: 100% !important;';
                
                // Desktop card styling - using individual style properties for better control
                cards.forEach((card, index) => {
                    ...
                    // First card doesn't need left margin
                    if (index === 0) {
    ...
    function handleResize() {
        if (isMobile()) {
            ...
            // Apply margin to first card
            const testimonialBlocks = document.querySelectorAll('.wp-block-bevision-client-testimonials');
            testimonialBlocks.forEach(block => {
                const carouselInner = block.querySelector('.carousel-inner');
                if (carouselInner && carouselInner.children.length > 0) {
                    carouselInner.children[0].style.marginLeft = '20px';
                }
                ...
                if (carousel) {
                    ...
                    cards.forEach((_, index) => {
                        ...
                        dot.addEventListener('click', () => {
                            ...
                            // Scroll to the card
                            const cardWidth = cards[0].offsetWidth + parseInt(window.getComputedStyle(cards[0]).marginRight || 30);
                            carousel.scrollTo({
                                left: index * cardWidth,
                                behavior: 'smooth'
                            });
    ...
    function setupAutoLoop() {
        ...
        testimonialBlocks.forEach((block, blockIndex) => {
            ...
            autoLoopIntervals[blockIndex] = setInterval(() => {
                    // Determine next slide (loop to beginning if at end)
                    let nextSlide = block.currentSlide + 1;
                    ...
                    // Calculate card width including margins - ensure we account for proper spacing
                    const cardWidth = cards[0].offsetWidth + 
                        parseInt(window.getComputedStyle(cards[0]).marginRight || 20);
                    
                    // Mobile devices need special handling for scroll positioning
                    if (isMobile()) {
                        // For mobile, we want to scroll precisely to the exact position of the selected card
                        const mobileCardPosition = nextSlide * cardWidth;
                        
                        // Smooth scroll to the next slide
                        carousel.scrollTo({
                            left: mobileCardPosition,
                            behavior: 'smooth'
                        });
                    } else {
                        // For desktop, continue with normal scrolling
                        carousel.scrollTo({
                            left: nextSlide * cardWidth,
                            behavior: 'smooth'
                        });
